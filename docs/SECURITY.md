...
| Idempotent approvals | The four admin decision endpoints — scan approve/reject and redemption approve/reject — claim the row with `UPDATE ... WHERE id = ? AND status = 'pending'` and treat `affectedRows` as the authority, so a repeated or concurrent decision moves no points and returns 409. See [ENGINEERING.md § 5](ENGINEERING.md#5-a-refund-that-could-be-collected-twice) |
